...
static void table_del_fill(table_del_t *t_del, unsigned int t_del_entries)
{
	...
	for (i = 0; i < t_del_entries; i++) {
		t_del[i].del_txnid = (rand() % 5) *2; // generate only pair numbers
		t_del[i].ROW_ID.otid = rand() % 10;
		t_del[i].ROW_ID.bucketProperty = 0;
		t_del[i].ROW_ID.rowId = rand() % 10;
	}
}
...
static void table_req_fill(table_req_t *t_req, unsigned int t_req_entries)
{
	...
	for (i = 0; i < t_req_entries; i++) {
		t_req[i].ROW_ID.otid = rand() % 10;
		t_req[i].ROW_ID.bucketProperty = 0;
		t_req[i].ROW_ID.rowId = rand() % 10;
	}
}
 ...
 */
	printf("\n>> BUILDING THE HASH TABLE BY BURSTS OF %d ENTRIES\n\n",
		TABLE_DEL_SIZE);
	...
	printf("\n>> LOOKING FOR %d ENTRIES BY BURSTS OF %d ENTRIES\n\n", 
